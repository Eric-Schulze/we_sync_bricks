...
func NewUserRepository(db models.DBService) *UserRepository {
...
func (r *UserRepository) GetUserByID(userID int64) (*models.User, error) {
	...
	if err != nil {
		logger.Error("Failed to scan user", "user_id", userID, "error", err)
		return nil, err
	}

	logger.Debug("Successfully retrieved user", "user_id", userID, "username", user.Username)
	return &user, nil
}

// GetUserProfile retrieves a user's profile by user ID (alias for GetUserByID for compatibility)
func (r *UserRepository) GetUserProfile(userID int64) (*models.User, error) {
	return r.GetUserByID(userID)
}

// UpdateUser updates a user's profile information
func (r *UserRepository) UpdateUser(userID int64, req *models.UserUpdateRequest) (*models.User, error) {
	logger.Debug("Updating user", "user_id", userID, "username", req.Username)
	...
	err = rows.Scan(&user.ID, &user.Username, &user.FirstName, &user.LastName, &user.Email, &user.CreatedAt, &user.UpdatedAt)
	if err != nil {
		logger.Error("Failed to scan updated user", "user_id", userID, "error", err)
		return nil, err
	}

	logger.Info("Successfully updated user", "user_id", userID, "username", req.Username)
	return &user, nil
}

// ChangePassword updates a user's password after verifying the current password
func (r *UserRepository) ChangePassword(userID int64, currentPassword, newPassword string) error {
...
func (r *UserRepository) GetUserOAuthCredentials(userID int64, provider string) (*models.UserOAuthCredential, error) {
	logger.Debug("Getting OAuth credentials", "user_id", userID, "provider", provider)

	sql := `SELECT id, user_id, provider, consumer_key, consumer_secret, token, token_secret, is_active, created_at, updated_at
			FROM user_oauth_credentials
			WHERE user_id = $1 AND provider = $2 AND is_active = true`

	credential, err := db.QueryRowToStructFromService[models.UserOAuthCredential](r.db, sql, userID, provider)
	if err != nil {
...
func (r *UserRepository) UpsertUserOAuthCredentials(userID int64, req *models.APIKeyUpdateRequest) error {
...
func (r *UserRepository) DeleteUserOAuthCredentials(userID int64, provider string) error {
